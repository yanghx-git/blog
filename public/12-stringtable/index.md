...
- String 实现了Serializable 接口：表示字符串是支持序列化的。实现了Comparable接口：表示string可以比较大小。
- String 在 jdk8 及以前内部定义了 final  char[] value 用于存储字符串数据。JDK9时改为byte[]

#### 1.2 为什么JDK9改变了结构

String类的当前实现将字符存储在char数组中，每个字符使用两个字节(16位)。从许多不同的应用程序收集的数据表明，字符串是堆使用的主要组成部分，而且，大多数字符串对象值包含拉丁字符。这些字符只需要一个字节的存储空间，因此这些字符串对象的内部 char数组中有一半的空间将不会使用。

我们建议改变字符串的内部表示class从utf-16字符串数组到字节数组 + 一个encoding-flag字段。新的String类将根据字符串的内容存储编码为ISO-8859-1/Latin-1（每个字符一个字节）或UTF-16（每个字符两个字节）的字符。编码标志将只是使用哪种编码。
...
同事基于String的数据结构，例如StringBuffer和StringBuilder也同样做了修改

#### 1.3 String的不可变性

String：代表不可变的字符序列。简称：不可变性。

> 当对字符串中心赋值是，需要重写指定内存区域赋值，不能使用原有的value进行赋值。当对现有的字符串进行连接操作时，也需要重新指定内存区域赋值，不能使用原有的value进行赋值。当调用string的replace()方法修改执行字符串或字符串时，也需要指定内存区域赋值，不能使用原有的value进行赋值。通过字面量的方式（区别于new）给一个字符串赋值，此时的字符串值声明在字符串常量池中。
...
![image-20200711093546398](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711093546398.png)

![image-20200711093558709](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711093558709.png)

#### 2.1 为什么StringTable从永久代调整到堆中

在JDK 7 中，interned字符串不再Java堆的永久生成中分配，而是在Java堆的主要部分（成为年轻代和老年代）中分配，与应用程序创建的其他对象一起分配。此更改将导致驻留在主Java堆中的数据更多，驻留在永久生成中的数据更少，因此可能需要调整堆大小。优于这一变化，大多数应用程序在堆使用方面只会看到相对较小的差异，但加载许多类或大量使用字符串的较大应用程序会出现这种差异。intern()方法会看到更显著的差异。
...
如果拼接符号的前后出现了变量，则相当于在堆空间new String(),具体的内容为拼接的结果

而调用intern方法，则会判断字符串常量池中是否存放在JavaEEhadoop值，如果存在则返回常量池中的值，否则就在常量池中创建。

#### 4.1 底层原理

拼接操作的底层其实使用了StringBuilder

![image-20200711102231129](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711102231129.png)
...
![image-20200711145925091](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711145925091.png)
...
![image-20200711150859709](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711150859709.png)
...
![image-20200711151326909](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711151326909.png)

针对下面这题，在JDK6和8中表现的是一样的

![image-20200711151433277](https://blog-1257196793.cos.ap-beijing.myqcloud.com/image-20200711151433277.png)
