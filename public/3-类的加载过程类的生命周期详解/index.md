...
![image-20210430215050746](https://blog-1257196793.cos.ap-beijing.myqcloud.com/b73db20eea59665fa3de67e1dec6d4e5.png)

其中，验证、准备、解析 3 个部分统称为链接（Linking）

从程序中类的使用过程看

![image-20210430215236716](https://blog-1257196793.cos.ap-beijing.myqcloud.com/c6e5a63339bb2574b9b52a9ed14ee9c5.png)
...
所谓加载，简而言之就是将Java类的字节码文件加载到机器内存中，并在内存中构建出Java类的原型----类模板对象。所谓模板对象，其实就是Java类在JVM内存中的一个快照，JVM将从字节码文件中解析出的常量池、类字段、类方法等信息存储到类模板中，这样JVM在运行期便能通过类末班而获取Java类中的任意信息，能够堆Java类的成员变量进行遍历，也能进行Java方法的调用。
...
- 在获取到类的二进制信息后，Java虚拟机就会处理这些数据，应最终转为一个java.lang.Class的实例。
...
![image-20210430221037898](https://blog-1257196793.cos.ap-beijing.myqcloud.com/10969de50477655b954b23bfe550fb1d.png)
...
创建数组类的情况下稍微有些特殊，因为数组类本身并不是由类加载器负责创建，而是由JVM在运行时根据需要而直接创建的，但数组的元素类型仍然需要依靠类加载器去创建。创建数组类（下述简称A）的过程：

- 如果数组的元素类型是引用类型，那么久尊徐定义的加载过程递归加载过程和创建数组A的元素类型；
- JVM使用指定的元素类型和数组维度来创建新的数组类。

如果数组的元素类型是引用类型，数组类的可访问性就由元素类型的可访问性决定。否则数据类型的可访问性被缺省定义为public。
...
![image-20210430221736546](https://blog-1257196793.cos.ap-beijing.myqcloud.com/2309ce7d4bdbffb86ae462e967af5dc9.png)

##### 3.1.1 整体说明

验证的内容则涵盖了数据信息的格式验证，语义检查，字节码验证，以及符号引用验证等。

- 其中格式验证会和加载阶段一起执行。验证童工之后，类加载器才会成功将类的二进制数据信息加载到方法区中。
- 格式验证室外的验证操作将会在方法区中进行。

连接阶段的验证虽然拖慢了加载速度，但是它避免了在字节码运行时还需要进行各种检查。（磨刀不误砍柴工）

##### 3.1.2 具体说明

1. 格式验证：是否以魔术 0XCAFEBABE 开头，主版本和副版本号是否在当前 Java 虚拟机的支持范围内，数据中每一个项是否都拥有正确的长度等。
...
![image-20210430225015932](https://blog-1257196793.cos.ap-beijing.myqcloud.com/b223d288b413f0ed0732357023a1dc7e.png)
...
![image-20210430235455086](https://blog-1257196793.cos.ap-beijing.myqcloud.com/5aaa031ab1ffc8071fb92a32ead888ef.png)
